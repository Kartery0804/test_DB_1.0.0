...
@regulate(0b1101)
def add_announce(conn:pymysql.Connection ,title: str,content:str,publisher_user_name :str,target_scope:str = None,target_ids:dict=None,publish_at :str=None,expire_at:str=None,read_required:int=None,status:str=None,**kwargs):
    try:
        dept = empl = []
        id_data = {"dept":[],"empl":[],"pos":[()]}
        if target_scope == "dept":
            dept = target_ids["dept"]
            for dp in dept:
                dept_data = om.mysql_select_dict(conn,"department",{"dept_name":dp})["data"]
                if dept_data:
                    id_data["dept"].append(dept_data[0][0])
        if target_scope == "employee":
            empl = target_ids["empl"]
            for em in empl:
                empl_data = om.mysql_select_dict(conn,"employee",{"employee_no":em})["data"]
                if empl_data:
                    id_data["empl"].append(empl_data[0][0])
        if target_scope == "position":
            pos = target_ids["pos"]
            for ps in pos:
                dp_name,pos_name =ps

        if publisher_user_name!=None:
            approver_user_id = om.mysql_select_dict(conn,"sys_user",{"username":publisher_user_name})['data'][0][0]
